...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AuthError ? t(`auth.${err.code}`) : (err instanceof Error ? err.message : t('auth.err_login_failed'));
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div style={{ minHeight: 'calc(100vh - 120px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#64748B', marginTop: 8 }}>{t('hero.subtitle')}</p>
        </div>
        ...
        <p style={{ textAlign: 'center', color: '#64748B', marginTop: 24, fontSize: 14 }}>
          {t('auth.no_account')}{' '}
          <Link to="/register" style={{ color: '#FFD60A', textDecoration: 'none', fontWeight: 600 }}>{t('auth.register_link')}</Link>
        </p>
      </div>
    </div>
  );
